...
int main(int argc, char ** argv){
  ...
  while (!all_packets_ack)
  {
    ...
    if(!(expired || retransmiting) && !feof(clientFile))
    {
      ...
      if (bytes_in_network + 512 > congestion_window)
      {
        ...
        if(receivedRecently) {
          outgoingPacket = Packet(sequenceNum, ackNum, connectionID, 1, 0, 0, fileData);
        ...
        std::cout << "Sending " << bytesRead << " with sequence num : " << sequenceNum - bytesRead << " with size " << bytesRead << std::endl;
      }
  ...
  std::cout << "numSent : " << numSent << "This is the FIN" << std::endl;
  ...
  while(1){
    ...
    if (pollstatus[0].revents & POLLIN){
      ...
      std::cout << "RECV " << p6.m_sequenceNum << " " << p6.m_ackNum << " " << p6.m_connectionID;
      if (p6.m_ackFlag == 1){ std::cout << " ACK"; }
      if (p6.m_synFlag == 1){ std::cout << " SYN"; }
      if (p6.m_finFlag == 1){ std::cout << " FIN"; }
      std::cout << std::endl;
  ...
  while (1){
    ...
    if (pollstatus[0].revents & POLLIN){
      ...
      if(p9.m_finFlag) {
        std::cout << "RECV " << p9.m_sequenceNum << " " << p9.m_ackNum << " " << p9.m_connectionID;
...
void closeClient(){
  std::cout << "Closing this bitch" << std::endl;
  exit(1);
}
